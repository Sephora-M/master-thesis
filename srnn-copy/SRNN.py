...
class SRNN_model(object):
    ...
    def __init__(self, num_classes, num_frames , num_temp_features, num_st_features, num_units, max_gradient_norm, learning_rate,
...
class LSTM_model(object):
    def __init__(self, num_classes, num_joints, num_frames , num_features_per_joints, num_units, num_layers, attention_lstm_num_units,attention_num_hidden_fc1, attention_placement, max_gradient_norm, learning_rate,
                 learning_rate_decay_factor, adam_epsilon,  GD, forward_only=False, l2_regularization=False, weight_decay=0, log_dir = None):
        """"
        Create LSTM model
        edgeRNNs: dictionary with keys as RNN name and value is a list of layers
        ...
        self.num_classes = num_classes
        self.num_features_per_joints = num_features_per_joints
        self.learning_rate = tf.Variable(float(learning_rate), trainable=False)
        self.learning_rate_decay_op = self.learning_rate.assign(
            self.learning_rate * learning_rate_decay_factor)
        self.max_grad_norm = max_gradient_norm
        ...
        infos = {}
        outputs = {}
        num_features = self.num_features_per_joints*num_joints
        attention_out_size=num_units
        attention_in_size=num_units
        if attention_placement == 0:
            attention_out_size=num_joints
            attention_in_size=num_features
        self.batch_size = tf.placeholder(tf.int32, name='batch_size')
        self.inputs = tf.placeholder(tf.float32, shape=(None, num_frames, num_features), name='joints_features')
        self.targets = tf.placeholder(tf.float32, shape=(None,num_classes), name='targets')

        for L in xrange(num_layers):
            single_cell = tf.nn.rnn_cell.LSTMCell(num_units, state_is_tuple=True)
            LSTM_layers[L] = single_cell #tf.nn.rnn_cell.MultiRNNCell([single_cell] * 3, state_is_tuple=True)
            states[L] = LSTM_layers[L].zero_state(self.batch_size, tf.float32)
            infos[L]={'input_gates' : [],'forget_gates' : [],'modulated_input_gates': [],'output_gates' : [], 'activations' : [], 'state_c' : [], 'state_m' : [], 'attention' : []}
            outputs[L] = []
        if attention_placement == num_layers:
            infos[attention_placement]={'input_gates' : [],'forget_gates' : [],'modulated_input_gates': [],'output_gates' : [], 'activations' : [], 'state_c' : [], 'state_m' : [], 'attention' : []}
        # weights = {'out' : tf.Variable(tf.random_normal([num_units*num_frames,num_classes]))}
        # biases = {'out' : tf.Variable(tf.random_normal([num_classes]))}
        attention_LSTM = tf.nn.rnn_cell.LSTMCell(attention_lstm_num_units, state_is_tuple=True)
        states['attention'] = attention_LSTM.zero_state(self.batch_size,tf.float32)

        weights = {
            'out': tf.Variable(tf.random_normal([num_units*num_frames,num_classes])),
            'attention_FC1': tf.Variable(tf.random_normal([attention_lstm_num_units+attention_in_size,attention_num_hidden_fc1])),
            'attention_FC2': tf.Variable(tf.random_normal([attention_num_hidden_fc1,attention_out_size]))
        }
        biases = {
            'out': tf.Variable(tf.random_normal([num_classes])),
            'attention_FC1': tf.Variable(tf.random_normal([attention_num_hidden_fc1])),
            'attention_FC2': tf.Variable(tf.random_normal([attention_out_size]))
        }
        ...
        def attention(x_t, x_t1):
            h_t1, states['attention'] = attention_LSTM(x_t1, states['attention'], scope='attention')
            fc1 = tf.matmul(tf.concat(1,[x_t, h_t1]),weights['attention_FC1']) + biases['attention_FC1']
            fc2 = tf.matmul(tf.tanh(fc1), weights['attention_FC2']) + biases['attention_FC2']
            tmp_at = sigmoid(fc2)
            if attention_placement == 0:
                at =  tf.pack([tmp_at]*num_features_per_joints,2)
        ...
        inputs = tf.split(0, num_frames, inputs)

        # connect the LSTM layers
        with tf.variable_scope("LSTM"):
            # for L in xrange(num_layers):
                # outputs, states = tf.nn.rnn(LSTM_layers[L], inputs,dtype=tf.float32, scope='LSTM'+str(L))
                # inputs = outputs
            for time_step in range(num_frames):
                if time_step > 0: tf.get_variable_scope().reuse_variables()
                inputs = self.inputs[:,time_step,:]
                if attention_placement==0:
                    inputs_att = inputs
                    if time_step > 0:
                        inputs_att_t1 = self.inputs[:, time_step-1, :]
                    else:
                        inputs_att_t1 = tf.zeros_like(inputs)
                for L in xrange(num_layers+1):
                    if L == attention_placement:
                        at_shaped, at  = attention(inputs_att, inputs_att_t1)
                        infos[L]['attention'].append(at)
                        inputs = tf.multiply(at_shaped, inputs)

                    if L < num_layers:
                        state =  states[L]
                        scope = "lstm_layer_"+str(L)
                        # i,j,f,o = get_gates(LSTM_layers[L],inputs, state, scope)
                        outputs_L, states[L] = LSTM_layers[L](inputs,state, scope=scope)
                        outputs[L].append(outputs_L)
                        # infos[L]['input_gates'].append(i)
                        # infos[L]['forget_gates'].append(f)
                        # infos[L]['modulated_input_gates'].append(j)
                        # infos[L]['output_gates'].append(o)
                        # infos[L]['activations'].append(outputs[L])
                        # infos[L]['state_c'].append(states[L][0])
                        # infos[L]['state_m'].append(states[L][1])
                        if self.save_summaries:
                            tf.summary.histogram('activations_'+L + '_time_step_' + str(time_step),outputs[L])
                            tf.summary.histogram('state_C_'+L + '_time_step_' + str(time_step),states[L][0])
                            tf.summary.histogram('state_M_'+L + '_time_step_' + str(time_step),states[L][1])
                            # tf.summary.histogram('input_gate'+L + '_time_step_' + str(time_step), i)
                            # tf.summary.histogram('modulated_input_gate'+L + '_time_step_' + str(time_step), j)
                            # tf.summary.histogram('forget_gate'+L + '_time_step_' + str(time_step), f)
                            # tf.summary.histogram('output_gate'+L + '_time_step_' + str(time_step), o)
                        inputs = outputs[L][time_step]  # the input to the next lstm cell is the output of the current

                    if L == attention_placement-1 : # in case attention placement > 0, take the input off att to be the output of prev LSTM layer
                        inputs_att = outputs[L][time_step]
                        if time_step > 0:
                            inputs_att_t1 = outputs[L][time_step-1]
                        else:
                            inputs_att_t1 = tf.zeros_like(outputs[L][time_step])


                final_outputs.append(inputs)

        self.infos = infos
        
        #output = tf.concat(1,outputs[-1], name="output_lastCells")
        self.final_states = states
        self.output = tf.concat(1,final_outputs, name="output_lastCells")
        self.logits = tf.matmul(self.output, weights['out'], name="logits") + biases['out']
        with tf.name_scope('cross_entropy'):
            loss = tf.nn.softmax_cross_entropy_with_logits(self.logits, self.targets)
            with tf.name_scope('total'):
                self.cost = tf.reduce_mean(loss)
    ...
    def step(self,session,inputs, targets, forward_only, batch_size, get_info = False):
        input_feed = {}
        input_feed[self.inputs.name] = inputs['all_joints_features']
        input_feed[self.targets.name] = targets
        input_feed[self.batch_size.name] = batch_size

        if not forward_only:
            output_feed = [self.updates, self.gradients_norm, self.cost, self.output, self.logits]
        else:
            if get_info:
                output_feed = [self.cost, tf.nn.softmax(self.logits), self.infos]
            else:
        ...
        if not forward_only:
            return outputs[1], outputs[2], None, outputs[3], outputs[4] # returns gradients norm and cost and no output
        else:
            if get_info:
                return  outputs[2], outputs[0], outputs[1]  # no gradients, cost and output
            else:
    ...
    def step_with_summary(self,session, inputs, targets, forward_only, batch_size):
        input_feed = {}
    ...
    def steps(self, session, all_data, batch_size):
        ...
        batch_losses= np.zeros(num_batches)
        batch_outputs= np.zeros((num_batches, batch_size, self.num_classes))
        missclassified = 0.0
        for i in range(num_batches):
            batch = self.get_batch(all_data,i,batch_size)
            _,batch_losses[i],batch_outputs[i] = self.step(session,batch[0],batch[1],True,batch_size)
            classes = np.argmax(batch_outputs[i],1)
            for d in range(batch_size):
                if batch[1][d][classes[d]] != 1:
                    missclassified += 1
        error = missclassified / (num_batches*batch_size)
        return np.mean(batch_losses), error
    ...
    def big_step(self, session, all_data,batch_size):
        ...
        _,loss,outputs = self.step(session,all_data[0],all_data[1],True,batch_size)
        classes = np.argmax(outputs,1)
        for d in range(size):
            if all_data[1][d][classes[d]] != 1:
                missclassified += 1
        error = missclassified / (size)
        return loss, error
