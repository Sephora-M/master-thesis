...
def get_pos_imgsJHMDB(joint_positions_path='/local/home/msephora/master-thesis/master-thesis/srnn-TF/data/JHMDB/joint_positions',
                      sub_activities=True, normalized=False, ind_split=1, subset_joints=True):
    if sub_activities:
...
def get_pos_imgsMRS(joint_positions_path='/local/home/msephora/master-thesis/master-thesis/data/MSRAction3D/MSRAction3DSkeleton(20joints)',subset_joints=True):
...
def get_pos_imgsNTU(joint_position_path='/local/home/msephora/master-thesis/master-thesis/srnn-TF/data/NTU_RGBD_dataset/nturgb+d_skeletons', cross_subject=False, subset_joints=True):
    train_split = ['C002','C003']

    if cross_subject:
        train_split_int = [ 1, 2, 4, 5, 8, 9, 13, 14, 15, 16, 17, 18, 19, 25, 27, 28, 31, 34, 35, 38]
        train_split = []
        for i in train_split_int:
            if i<10:
                train_split.append('P00'+str(i))
            else:
                train_split.append('P0'+str(i))
...
def get_pos_imgsUTK(joint_positions_path='/local/home/msephora/master-thesis/master-thesis/srnn-TF/data/UTKinect-Action3D_Dataset/joints_per_action', subset_joints=True):
    train_split = ['s01','s03','s05','s07','s09']
...
def extract_all_features(all_data, num_video, num_activities, num_considered_frames, num_joints, JHMDB=False):
    print("Num videos:")
    print(num_video)
    ...
    features["all_joints_features"] = np.zeros((num_video,num_considered_frames,NUM_TEMP_FEATURES*num_joints))
    current_video = 0
    for action_id in all_data:
        for video in all_data[action_id]:
            infos[current_video] = [action_id, video]
            features["all_joints_features"][current_video,:,:] = extract_all_ntraj(all_data[action_id][video], num_joints, num_considered_frames,JHMDB)
            current_video += 1
    ...
    return [features, action_classes, infos]
...
def extract_temp_features(pos_img, joint_id, num_frames):
    ...
    temp_features = np.zeros((num_frames, 3))
...
def extract_all_ntraj(pos_img, num_joints, num_frames, JHMDB):
    ...
    if JHMDB:
        ...
        NUM_TEMP_FEATURES = 4
    relative_pos = normalize_positions(pos_img, NUM_TEMP_FEATURES,num_joints)
    frames_chosen = np.linspace(0, tot_num_frames - 1, num_frames).astype(int)

    temp_features = np.zeros((num_frames, NUM_TEMP_FEATURES*num_joints))
    ...
    for i in range(num_frames-1):
        for joint_id in xrange(num_joints):
            ...
            #ort = math.atan2(d[1],d[0])*180.0/math.pi
            if not JHMDB:
                idx_feat = joint_id*NUM_TEMP_FEATURES
                temp_features[i,0+idx_feat] = relative_pos[0,joint_id,frames_chosen[i]] #  relative x
                temp_features[i,1+idx_feat] = relative_pos[1,joint_id,frames_chosen[i]] #  relative y
                temp_features[i,2+idx_feat] = relative_pos[2,joint_id,frames_chosen[i]] #  relative z
                temp_features[i,3+idx_feat] = dist
            else:
                idx_feat = joint_id*NUM_TEMP_FEATURES
                temp_features[i,0+idx_feat] = relative_pos[0,joint_id,frames_chosen[i]] #  relative x
                temp_features[i,1+idx_feat] = relative_pos[1,joint_id,frames_chosen[i]] #  relative y
                temp_features[i,2+idx_feat] = dist
    for joint_id in xrange(num_joints):
        if not JHMDB:
            idx_feat = joint_id*NUM_TEMP_FEATURES
            temp_features[num_frames - 1,0+idx_feat] = relative_pos[0,joint_id,frames_chosen[num_frames - 1]] #  realative x
            temp_features[num_frames - 1,1+idx_feat] = relative_pos[1,joint_id,frames_chosen[num_frames - 1]] #  reative y
            temp_features[num_frames - 1,2+idx_feat] = relative_pos[2,joint_id,frames_chosen[num_frames - 1]] #  reative z
            temp_features[num_frames - 1,3+idx_feat] = 0.0
        else:
            idx_feat = joint_id*NUM_TEMP_FEATURES
            temp_features[num_frames - 1,0+idx_feat] = relative_pos[0,joint_id,frames_chosen[num_frames - 1]] #  realative x
            temp_features[num_frames - 1,1+idx_feat] = relative_pos[1,joint_id,frames_chosen[num_frames - 1]] #  reative y
            temp_features[num_frames - 1,2+idx_feat] = 0.0
...
def normalize_positions(pos_img,num_temp_feats,num_joints):
    ...
    return pos_img - np.tile(torso_positions, [1 ,num_joints, 1])
