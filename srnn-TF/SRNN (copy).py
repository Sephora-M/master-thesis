...
class SRNN_model(object):
    ...
    def __init__(self, num_classes, num_frames , num_units, max_gradient_norm, batch_size, learning_rate,
        ...
        for temp_feat in self.temp_features_names:
            self.inputs[temp_feat] = tf.placeholder(tf.float32, shape=(None,self.num_frames, self.num_features), name=temp_feat)
        ...
        for st_feat in self.st_features_names:
            self.inputs[st_feat] = tf.placeholder(tf.float32, shape=(None,self.num_features, self.num_features), name=st_feat)
            edgesRNN[st_feat] = tf.nn.rnn_cell.LSTMCell(num_units)
            states[st_feat] = edgesRNN[st_feat].zero_state(batch_size,tf.float32)

        for node in nodes_names:
            self.inputs[node] = tf.placeholder(tf.float32, shape=(None,self.num_features, self.num_features), name=node)
        ...
        with tf.variable_scope("SRNN"):
            ...
            for time_step in range(num_frames):
                ...
                for temp_feat in self.temp_features_names:
                    # temp_input = tf.reshape(self.inputs[temp_feat][:,time_step], [self.batch_size, 1])
                ...
                for st_feat in self.st_features_names:
                    # st_input = tf.reshape(self.inputs[st_feat][:,time_step], [self.batch_size, 1])
                ...
                #node_inputs['face'] = tf.reshape(node_inputs['face'],[self.batch_size, 1])
                ...
                #node_inputs['belly'] = tf.reshape(node_inputs['face'],[self.batch_size, 1])
                ...
                #node_inputs['arms'] = tf.reshape(node_inputs['arms'],[self.batch_size, 1])
                ...
                #node_inputs['legs'] = tf.reshape(node_inputs['legs'],[self.batch_size, 1])
    ...
    def step(self,session,temp_inputs, st_inputs, targets, forward_only):
        ...
        for temp_features_name in self.temp_features_names:
            input_feed[temp_features_name] = temp_inputs[temp_features_name]

        for st_features_name in self.st_features_names:
            input_feed[st_features_name] = st_inputs[st_features_name]
